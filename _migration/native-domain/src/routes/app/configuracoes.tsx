...
function ConfigPage() {
  ...
  async function saveSenha() {
    ...
    setSavingS(true);
    const { error } = await supabase.auth.updateUser({ password: senha.nova, currentPassword: senha.atual });
    setSavingS(false);
    if (error) return toast.error(error.message);
...
<div><Label>Senha atual</Label><Input type="password" autoComplete="current-password" value={senha.atual} onChange={(e) => setSenha({ ...senha, atual: e.target.value })} /></div>
