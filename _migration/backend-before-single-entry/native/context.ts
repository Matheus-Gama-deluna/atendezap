...
export async function makeContext(request:Request,env:Record<string,string>,publicWebhook=false){
 ...
 admin.auth={admin:{
  ...
  async getUserById(id:string){const row=(await blink.db.sql('SELECT user_id,email FROM profiles WHERE user_id=?',[id])).rows[0];return{data:{user:row?{id:row.user_id,email:row.email}:null},error:null}},
 }}
 for(const db of [admin,scoped])db.rpc=async(name:string,args:any={})=>{try{
  ...
  if(name==='fin_enable_for_company'){
   ...
   await blink.db.sql('UPDATE company SET financeiro_ativo=? WHERE id=?',[Number(!!args._enable),cid]);return{data:true,error:null}
  }
  ...
  if(db.internal&&name==='consume_ai_credit'){const rows=(await blink.db.sql('UPDATE company SET creditos_saldo=creditos_saldo-1 WHERE id=? AND creditos_saldo>0 RETURNING creditos_saldo',[cid])).rows;return{data:rows.length>0,error:null}}
  if(db.internal&&name==='refund_ai_credit'){await blink.db.sql('UPDATE company SET creditos_saldo=creditos_saldo+1 WHERE id=?',[cid]);return{data:true,error:null}}
  if(db.internal&&name==='topup_plan_credits'){await blink.db.sql('UPDATE company SET creditos_saldo=COALESCE((SELECT creditos_mensais FROM plan WHERE slug=?),0),creditos_origem=? WHERE id=?',[args._plan_slug,'plano',cid]);return{data:true,error:null}}
