...
function ConversasPage() {
  ...
  const observedMessages = useRef<{companyId:string;ids:Set<string>;ready:boolean}>({companyId:"",ids:new Set(),ready:false});
  ...
  useRealtimePolling(!!companyId, () => companyId ? load(companyId) : undefined, 5000);
  ...
  async function load(cid: string) {
    ...
    const incoming = (m ?? []) as Msg[];
    const observed = observedMessages.current;
    if (observed.ready && observed.companyId === cid) {
      const fresh = incoming.filter(message => !observed.ids.has(message.id) && message.direcao === "entrada" && message.numero !== active);
      if (fresh.length) setUnread(previous => {const next={...previous};for(const message of fresh)next[message.numero]=(next[message.numero]??0)+1;return next});
    }
    observedMessages.current={companyId:cid,ids:new Set(incoming.map(message=>message.id)),ready:true};
    setMsgs(incoming);
    ...
    <div className="space-y-4">
              ...
              <header className="flex items-center gap-3 px-4 py-3 border-b border-[color:var(--hairline)] bg-[color:var(--panel)]">
                ...
                <div className="ml-auto flex items-center gap-3 flex-wrap">
                  ...
                  <Button size="sm" variant="outline" onClick={async () => {
                    if (!active) return;
                    try {
                      const result = await sendCsatFn({ data: { numero: active, contatoNome: activeConv?.nome ?? null } });
                      if (result.sent) toast.success("Pesquisa de satisfação enviada");
                      else toast.warning("Pesquisa criada, mas não enviada. Confira a conexão do WhatsApp e tente novamente.");
      ...
      {companyId && (
        <LeadDrawer
          ...
          onChanged={() => companyId ? load(companyId) : undefined}
        />
      )}
    </div>
  );
